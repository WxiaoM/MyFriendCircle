...
@interface OCTCountTextView()<UITextViewDelegate,UIScrollViewDelegate>
{
...
@implementation OCTCountTextView
...
- (void)createUI{
    ...
    [self.textView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(ScreenWidth-65);
    }];
...
- (void)keyboardWillShow:(NSNotification *)aNotification
{
    ...
    if (self.textView.text.length == 0) {
        ...
        self.backGroundView.frame = CGRectMake(0, ScreenHeight-height-49, ScreenWidth, 49);
    }else{
        CGRect rect = CGRectMake(0, ScreenHeight - self.backGroundView.frame.size.height-height, ScreenWidth, self.backGroundView.frame.size.height);
...
{
    if (self.textView.text.length == 0) {
        self.backGroundView.frame = CGRectMake(0, 0, ScreenWidth, 49);
        self.frame = CGRectMake(0, ScreenHeight-49, ScreenWidth, 49);
    }else{
        CGRect rect = CGRectMake(0, 0, ScreenWidth, self.backGroundView.frame.size.height);
        self.backGroundView.frame = rect;
        self.frame = CGRectMake(0, ScreenHeight - rect.size.height, ScreenWidth, self.backGroundView.frame.size.height);
    }
}
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    //---- 计算高度 ---- //
    CGSize size = CGSizeMake(ScreenWidth-65, CGFLOAT_MAX);
    ...
    if (curheight < 19.094) {
        statusTextView = NO;
        self.backGroundView.frame = CGRectMake(0, y - 49, ScreenWidth, 49);
    }else if(curheight < MaxTextViewHeight){
        statusTextView = NO;
        self.backGroundView.frame = CGRectMake(0, y - textView.contentSize.height-10, ScreenWidth,textView.contentSize.height+10);
    }else{
        statusTextView = YES;
        return;
    }
...
- (void)sendClick:(UIButton *)sender{
    ...
    if (self.OCTCountTextViewBlock) {
        self.OCTCountTextViewBlock(self.textView.text);
    }
...
- (UIView *)backGroundView{
    if (!_backGroundView) {
        _backGroundView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, 49)];
